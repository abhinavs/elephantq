...
- `elephantq.configure(...)` is now `async`. Callers must write `await elephantq.configure(...)`. The sync variant could not await the prior global app's `close()` before replacing it, which leaked the asyncpg pool on every reconfigure (visible in test suites and hot-reload dev workflows). `configure` now awaits `close()` on the outgoing app so the pool is properly released.
...
- Feature helpers (`elephantq.enqueue`, `elephantq.schedule`, `elephantq.run_worker`, management functions, and the recurring scheduler's backend resolver) now honor the enclosing `ElephantQ(...)` instance via a new `_active_app` ContextVar. Calls made from inside an instance method route to that instance; calls from anywhere else use the global app as before. Previously a user with `app = ElephantQ(database_url="...A")` calling a global helper silently wrote to the global app's database instead of DB A.
...
- `MigrationRunner` serializes concurrent runs behind a session-scoped `pg_advisory_lock`. Two deploy nodes calling `elephantq setup` simultaneously no longer race on non-idempotent DDL or double-insert into `elephantq_migrations`; the losing node waits, re-reads the applied set, and is a no-op.
- `_ensure_postgres_database_exists` rejects database names that aren't valid SQL identifiers (`[A-Za-z_][A-Za-z0-9_$]*`) with a clear `ValueError` instead of producing quoting-level Postgres errors. Operators needing unusual database names should pre-create the database out of band.
