"""
Enhanced job processor with timeout support for Soniq.
...
try:
    _proc = importlib.import_module("soniq.core.processor")
...
async def get_job_timeout(job_id: str, conn: asyncpg.Connection) -> Optional[int]:
    ...
    try:
        timeout_data = await conn.fetchrow(
            """
            SELECT timeout_seconds FROM soniq_job_timeouts 
            WHERE job_id = $1
        """,
            job_id,
        )
...
async def store_job_timeout(
    job_id: str, timeout_seconds: int, conn: asyncpg.Connection
) -> bool:
    """
    ...
    try:
        ...
        await conn.execute(
            """
            INSERT INTO soniq_job_timeouts (job_id, timeout_seconds)
            VALUES ($1, $2)
...
async def execute_job_with_timeout(
    job_func, args: Dict[str, Any], timeout_seconds: int
) -> Any:
    """
    ...
    try:
        ...
        result = await asyncio.wait_for(job_func(**args), timeout=timeout_seconds)
        return result

    except asyncio.TimeoutError:
        raise JobTimeoutError(
            f"Job execution exceeded {timeout_seconds} seconds timeout"
        )
...
async def process_jobs_with_timeout(
    conn: asyncpg.Connection, queue: str = "default"
) -> bool:
    """
    ...
    skip_update_lock = os.environ.get("SONIQ_SKIP_UPDATE_LOCK", "").lower() in {
        "1",
        "true",
        "yes",
        "on",
    ...
    async with conn.transaction():
        ...
        logger.info(
            f"Processing job {job_id} ({job_name}) - attempt {attempts + 1}/{max_attempts}{timeout_info}"
        )
        ...
        try:
            ...
            if timeout_seconds:
                await execute_job_with_timeout(
                    job_meta["func"], validated_args, timeout_seconds
                )
            else:
            ...
            if new_attempts >= max_attempts:
                ...
                logger.error(
                    f"Job {job_id} moved to dead letter queue after timing out {new_attempts} times"
                )
            else:
...
async def run_worker_with_timeout(
    ...
    from soniq.settings import SONIQ_DATABASE_URL

    db_url = database_url or SONIQ_DATABASE_URL
    try:
    ...
    logger.info(
        f"Starting Soniq worker with timeout support - concurrency: {concurrency}, queues: {queues}"
    )
    ...
    try:
            ...
            async def worker():
                ...
                try:
                    ...
                    await listen_conn.add_listener(
                        "soniq_new_job", notification_callback
                    )
                    ...
                    while True:
                        try:
                            ...
                            async with pool.acquire() as job_conn:
                                for queue in queues:
                                    processed = await process_jobs_with_timeout(
                                        job_conn, queue
                                    )
                                    if processed:
                                        any_processed = True
        ...
        if pool:
            await pool.close()
        logger.info("Soniq timeout worker stopped")
...
def enable_timeout_processing():
    ...
    import soniq.core.processor

    soniq.core.processor.process_jobs = process_jobs_with_timeout

    logger.info("Soniq timeout processing enabled")
...
def disable_timeout_processing():
    ...
    soniq.core.processor.process_jobs = base_process_jobs

    logger.info("Soniq timeout processing disabled")
...
class TimeoutConfiguration:
    ...
    async def set_global_timeout(
        timeout_seconds: int, database_url: Optional[str] = None
    ):
        """
        ...
        try:
            async with pool.acquire() as conn:
                ...
                await conn.execute(
                    """
                    INSERT INTO soniq_config (key, value)
            ...
            async with pool.acquire() as conn:
                try:
                    result = await conn.fetchrow(
                        """
                        SELECT value FROM soniq_config 
