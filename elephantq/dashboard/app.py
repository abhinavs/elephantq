...
async def get_recent_jobs(limit: int = 50, queue: Optional[str] = None) -> List[Dict[str, Any]]:
        ...
        return [
            {
                ...
                "success_rate": round((row["completed_count"] / row["total_count"]) * 100, 2) if row["total_count"] > 0 else 0,
...
async def search_jobs(
    ...
    async with pool.acquire() as conn:
        ...
        if query:
            param_count += 1
            conditions.append(f"(job_name ILIKE ${param_count} OR last_error ILIKE ${param_count})")
...
async def get_system_health() -> Dict[str, Any]:
    ...
    async with pool.acquire() as conn:
        ...
        # Get queue backlog
        backlog = await conn.fetchval("SELECT COUNT(*) FROM elephantq_jobs WHERE status = 'queued'")
        ...
        if recent_stats["total_recent"] > 0:
            error_rate = (recent_stats["failed_recent"] / recent_stats["total_recent"]) * 100
