...
class DeadLetterJob:
    ...
    @classmethod
    def from_job_record(cls, job_record: Dict[str, Any], reason: DeadLetterReason) -> 'DeadLetterJob':
        """Create dead letter job from regular job record"""
        return cls(
            id=str(job_record['id']),
            job_name=job_record['job_name'],
            args=json.loads(job_record['args']) if isinstance(job_record['args'], str) else job_record['args'],
...
class DeadLetterManager:
    ...
    async def move_job_to_dead_letter(self, job_id: str, reason: DeadLetterReason, 
        ...
        async with pool.acquire() as conn:
            async with conn.transaction():
                ...
                dead_letter_job = DeadLetterJob.from_job_record(dict(job_record), reason)
                if tags:
    ...
    async def resurrect_job(self, dead_letter_id: str, reset_attempts: bool = True,
                           new_max_attempts: Optional[int] = None,
                           new_priority: Optional[int] = None,
                           new_queue: Optional[str] = None) -> Optional[str]:
        """Resurrect a job from the dead letter queue"""
        pool = await get_context_pool()
        async with pool.acquire() as conn:
            async with conn.transaction():
                # Get dead letter job
                dead_job = await conn.fetchrow(f"""
                ...
                if not job_meta:
                    logger.error(f"Cannot resurrect job {dead_letter_id}: job {dead_job['job_name']} not registered")
                    return None
    ...
    async def bulk_resurrect(self, filter_criteria: DeadLetterFilter, 
                           reset_attempts: bool = True,
                           new_max_attempts: Optional[int] = None) -> List[str]:
    ...
    async def list_dead_letter_jobs(self, filter_criteria: Optional[DeadLetterFilter] = None) -> List[DeadLetterJob]:
    ...
    async def get_dead_letter_stats(self, hours: Optional[int] = None) -> DeadLetterStats:
        ...
        async with pool.acquire() as conn:
            ...
            params = []
            if hours:
                time_condition = "WHERE moved_to_dead_letter_at >= NOW() - INTERVAL '%s hours'"
                params.append(hours)
            ...
            # By job name
            by_job_name = await conn.fetch(f"""
            ...
            # Oldest job age
            oldest_job = await conn.fetchval(f"""
            ...
            if resurrection_stats['total_resurrections'] > 0:
                resurrection_success_rate = (
                    ...
                    resurrection_stats['total_resurrections'] * 100
                )
            ...
            return DeadLetterStats(
                ...
                by_reason={row['dead_letter_reason']: row['count'] for row in by_reason},
    ...
    async def cleanup_old_dead_letter_jobs(self, days: int = 30) -> int:
        ...
        async with pool.acquire() as conn:
            ...
            deleted_count = int(result.split()[-1])
            logger.info(f"Cleaned up {deleted_count} dead letter jobs older than {days} days")
            return deleted_count
    ...
    async def add_tags_to_job(self, dead_letter_id: str, tags: Dict[str, str]) -> bool:
        ...
        async with pool.acquire() as conn:
            # Get current tags
            current_tags = await conn.fetchval(f"""
            ...
            result = await conn.execute(f"""
                ...
                SET tags = $1
                WHERE id = $2
            """, json.dumps(current_tags), uuid.UUID(dead_letter_id))
            
            return result.split()[-1] == "1"
    
    async def export_dead_letter_jobs(self, filter_criteria: Optional[DeadLetterFilter] = None,
                                     format: str = "json") -> str:
        ...
        if format.lower() == "json":
            ...
            filename = f"dead_letter_export_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
            ...
            import csv
            filename = f"dead_letter_export_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
...
async def move_job_to_dead_letter(job_id: str, reason: DeadLetterReason, 
                                 tags: Optional[Dict[str, str]] = None) -> bool:
    """Move a job to the dead letter queue"""
    return await _dead_letter_manager.move_job_to_dead_letter(job_id, reason, tags)


async def resurrect_job(dead_letter_id: str, reset_attempts: bool = True,
                       new_max_attempts: Optional[int] = None,
                       new_priority: Optional[int] = None,
                       new_queue: Optional[str] = None) -> Optional[str]:
    """Resurrect a job from the dead letter queue"""
...
async def bulk_resurrect_jobs(filter_criteria: DeadLetterFilter, 
                             reset_attempts: bool = True,
                             new_max_attempts: Optional[int] = None) -> List[str]:
    """Resurrect multiple jobs matching filter criteria"""
    return await _dead_letter_manager.bulk_resurrect(filter_criteria, reset_attempts, new_max_attempts)
...
async def list_dead_letter_jobs(filter_criteria: Optional[DeadLetterFilter] = None) -> List[DeadLetterJob]:
    """List dead letter jobs with optional filtering"""
...
async def export_dead_letter_jobs(filter_criteria: Optional[DeadLetterFilter] = None,
                                 format: str = "json") -> str:
    """Export dead letter jobs to file"""
