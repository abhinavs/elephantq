    ...
    def at_time(self, time_str: str) -> "JobScheduleBuilder":
        ...
        try:
            ...
            try:
                ...
                raise ValueError(f"Invalid time format: {time_str}. Use ISO format or HH:MM")
            ...
            'jobs': [
                {
                    ...
                    'scheduled_at': job._scheduled_at.isoformat() if job._scheduled_at else None,
...
async def schedule(job_func: Callable[..., Any], when: Union[datetime, int, float, timedelta], **kwargs) -> str:
    """
    ...
    if isinstance(when, datetime):
        ...
        return await schedule_job(job_func).in_seconds(int(when.total_seconds())).enqueue(**kwargs)
    else:
        raise TypeError(f"Unsupported schedule type: {type(when)}. Use datetime, int/float seconds, or timedelta")
...
async def schedule_high_priority(job_func: Callable[..., Any], **kwargs) -> str:
    ...
    return await schedule_job(job_func).with_priority(1).in_queue("urgent").enqueue(**kwargs)


async def schedule_background(job_func: Callable[..., Any], **kwargs) -> str:
    """Schedule a low priority background job (immediate execution)"""
    return await schedule_job(job_func).with_priority(100).in_queue("background").enqueue(**kwargs)
...
async def demo_advanced_scheduling():
    ...
    job_id1 = await schedule_job(example_scheduled_job).in_minutes(30).with_priority(5).enqueue(
    ...
    job_id2 = await schedule_job(example_scheduled_job).at_time(tomorrow_9am.isoformat()).enqueue(
        message="Tomorrow at 9 AM"
    )
    print(f"Scheduled for tomorrow 9 AM: {job_id2}")
    
    # Conditional scheduling
    job_id3 = await schedule_job(example_scheduled_job).in_hours(2).if_condition(
        lambda: True  # Always true for demo
    ).with_tags("conditional", "demo").enqueue(
        message="Conditional job"
    )
    ...
    batch.add_job(example_scheduled_job).in_minutes(5).with_priority(10).with_tags("batch1")
    batch.add_job(example_scheduled_job).in_minutes(10).with_priority(20).with_tags("batch2")
    batch.add_job(example_scheduled_job).in_minutes(15).with_priority(30).with_tags("batch3")
    ...
    # Dry run example
    print("\n3. Dry run preview:")
    config = await schedule_job(example_scheduled_job).in_days(1).with_priority(1).dry_run().enqueue(
        message="Dry run job"
    )
    ...
    urgent_job = await schedule_high_priority(example_scheduled_job, message="Urgent task")
    background_job = await schedule_background(example_scheduled_job, message="Background task")
