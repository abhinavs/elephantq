...
async def store_job_dependencies(job_id: str, dependencies: List[str], 
                               dependency_timeout: Optional[int] = None) -> bool:
    """
    ...
    async with pool.acquire() as conn:
        async with conn.transaction():
            ...
            for dep_job_id in dependencies:
                ...
                """, uuid.UUID(job_id), dep_uuid, timeout_at)
    
    return True


async def add_job_dependency(job_id: str, depends_on_job_id: str, dependency_timeout: Optional[int] = None) -> bool:
    ...
    async with pool.acquire() as conn:
        ...
        for dep in dependencies:
            ...
            if dep['timeout_at'] and datetime.now() > dep['timeout_at']:
                ...
                """, uuid.UUID(job_id), dep['depends_on_job_id'])
                continue
            ...
            'dependencies': [
                {
                    ...
                    'timeout_at': dep['timeout_at'].isoformat() if dep['timeout_at'] else None,
                    'job_created_at': dep['dep_created_at'].isoformat() if dep['dep_created_at'] else None,
                    'job_updated_at': dep['dep_updated_at'].isoformat() if dep['dep_updated_at'] else None,
                }
                for dep in dependencies
            ],
            'dependents': [
                {
                    ...
                    'created_at': dep['created_at'].isoformat() if dep['created_at'] else None,
                    'job_created_at': dep['dependent_created_at'].isoformat() if dep['dependent_created_at'] else None,
                }
