...
class MetricsCollector:
    ...
    def __init__(self, retention_hours: int = 24):
        ...
        self.queue_throughput: Dict[str, deque] = defaultdict(lambda: deque(maxlen=1000))
    ...
    async def record_job_completion(self, job_id: str, status: str, duration_ms: float, 
                                   memory_usage_mb: Optional[float] = None,
                                   cpu_usage_percent: Optional[float] = None):
    ...
    async def get_processing_time_percentiles(self, queue: str) -> Dict[str, float]:
        ...
        return {
            ...
            "p99": statistics.quantiles(times, n=100)[98] if len(times) >= 100 else max(times, default=0)
        }
...
class MetricsAnalyzer:
    ...
    async def get_system_metrics(self, timeframe_hours: int = 1) -> SystemMetrics:
        ...
        async with pool.acquire() as conn:
            ...
            success_rate = (done_count / total_completed * 100) if total_completed > 0 else 0.0
            ...
            throughput = total_jobs / timeframe_hours * 60 if timeframe_hours > 0 else 0.0
    ...
    async def _get_queue_stats(self, conn: asyncpg.Connection, timeframe_hours: int) -> List[QueueStats]:
    ...
    async def _generate_recommendations(self, metrics: SystemMetrics) -> List[str]:
        ...
        if metrics.success_rate < 95:
            recommendations.append(f"Success rate is {metrics.success_rate:.1f}% - investigate error patterns")
        
        # Check processing times
        if metrics.p95_processing_time_ms > 30000:  # 30 seconds
            recommendations.append("95th percentile processing time is high - consider job optimization")
        
        # Check queue backlogs
        for queue_stat in metrics.queue_stats:
            if queue_stat.queued_count > 1000:
                recommendations.append(f"Queue '{queue_stat.queue_name}' has {queue_stat.queued_count} backlogged jobs")
        
        # Check throughput
        if metrics.throughput_per_minute < 10:
            recommendations.append("Low throughput - consider increasing worker concurrency")
        
        # Check memory usage
        if metrics.memory_usage_mb > 4096:  # 4GB
            recommendations.append(f"High memory usage ({metrics.memory_usage_mb:.0f}MB) - monitor for memory leaks")
...
class AlertManager:
    ...
    async def check_alerts(self) -> List[Dict]:
        ...
        if metrics.p95_processing_time_ms > self.alert_thresholds["p95_processing_time_max_ms"]:
    ...
    def _should_send_alert(self, alert_key: str, current_time: float, cooldown_minutes: int = 30) -> bool:
...
async def record_job_completion(job_id: str, status: str, duration_ms: float, 
                               memory_usage_mb: Optional[float] = None,
                               cpu_usage_percent: Optional[float] = None):
...
async def export_metrics_json(timeframe_hours: int = 24, file_path: str = "metrics_export.json"):
