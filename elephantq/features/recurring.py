...
class EnhancedRecurringManager:
    ...
    def _calculate_next_run(
        self, schedule_type: str, schedule_value: Union[str, int], current_time: datetime
    ) -> Optional[datetime]:
        if schedule_type == "interval":
...
class EnhancedRecurringScheduler:
    ...
    async def start(self):
        ...
        print(f"Enhanced recurring scheduler started (check interval: {self.check_interval}s)")
    ...
    async def _execute_job(self, job_id: str, job: Dict[str, Any], current_time: datetime):
        """Execute a due job"""
        try:
            ...
            print(f"Recurring job {job_id} enqueued as {actual_job_id}, next run: {next_run}")
...
def get_scheduler_status() -> Dict[str, Any]:
    ...
    return {
        ...
        'active_jobs': len([j for j in _enhanced_manager.jobs.values() if j['status'] == 'active']),
        'paused_jobs': len([j for j in _enhanced_manager.jobs.values() if j['status'] == 'paused']),
