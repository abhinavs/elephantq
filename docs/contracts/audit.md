...
- Long-running commands (`soniq start`, `soniq scheduler`, `soniq dashboard`) are `manual-only` even though they accept `--help`. The smoke job invokes `--help` separately if it wants to assert the parser builds; the rows above describe the *command* itself.
