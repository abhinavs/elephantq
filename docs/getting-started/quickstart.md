...
This creates all required tables (`elephantq_jobs`, heartbeat tracking, recurring jobs, etc.) and is safe to re-run on every deploy.

## 5. Start a worker

```bash
ELEPHANTQ_DATABASE_URL="postgresql://localhost/myapp" \
ELEPHANTQ_JOBS_MODULES="jobs" \
elephantq start --concurrency 4
```

`ELEPHANTQ_JOBS_MODULES` tells the worker where to find your job definitions. It accepts comma-separated Python module paths, so a larger project might use `"app.jobs,billing.tasks,notifications.handlers"`. The current directory is automatically added to `sys.path`, so running the command from your project root is enough -- no `PYTHONPATH` gymnastics required.

!!! tip "Try without Postgres"
    For quick experiments, use SQLite -- no server needed:
    `ElephantQ(database_url="local.db")` (requires `pip install elephantq[sqlite]`).
...
- **Use environment variables** instead of hardcoding `database_url`. ElephantQ reads `ELEPHANTQ_DATABASE_URL` automatically.
...
- **Enable feature flags** for the capabilities you need. All features are off by default. The most common production flags:
    - `ELEPHANTQ_DEAD_LETTER_QUEUE_ENABLED=true` -- inspect permanently failed jobs instead of losing them.
    - `ELEPHANTQ_METRICS_ENABLED=true` -- expose Prometheus counters (requires `pip install elephantq[monitoring]`).
    - `ELEPHANTQ_LOGGING_ENABLED=true` -- structured JSON logging for log aggregators.
    - `ELEPHANTQ_TIMEOUTS_ENABLED=true` -- enforce per-job execution time limits.

See [installation.md](installation.md) for the full list of extras and feature flags.

## Delivery semantics

ElephantQ provides **at-least-once delivery**. A job may execute more than once if a worker crashes after running the handler but before marking the job as complete. Design your job functions to be idempotent -- use database upserts, deduplication keys, or idempotency tokens for side effects like emails or payments.
