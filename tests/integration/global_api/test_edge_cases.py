...
async def test_malformed_job_data():
    ...
    job_id = await elephantq.enqueue(test_simple_job, message="normal job")
...
async def test_concurrent_job_processing():
    ...
    for i in range(5):  # Reduced for speed
        job_id = await elephantq.enqueue(simple_job, message=f"concurrent job {i}")
...
async def test_job_argument_validation_edge_cases():
    ...
    status = await elephantq.get_job_status(valid_job_id)
    assert status["status"] == "done"
...
async def test_exception_handling_in_jobs():
    ...
    value_status = await elephantq.get_job_status(value_error_job)
    type_status = await elephantq.get_job_status(type_error_job)
...
async def test_worker_shutdown_handling():
    ...
    # Enqueue a quick job
    await elephantq.enqueue(simple_job, message="shutdown test")
...
async def test_environment_variable_handling():
    ...
    try:
        ...
        os.environ["ELEPHANTQ_DATABASE_URL"] = test_url

        # Verify environment variable is read correctly
        assert os.environ.get("ELEPHANTQ_DATABASE_URL") == test_url
