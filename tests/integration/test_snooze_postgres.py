...
async def test_snooze_requeues_with_attempts_unchanged_against_postgres():
    ...
    @elephantq.job(retries=1, name="snooze_then_succeed")
    ...
    job_id = await elephantq.enqueue(snooze_then_succeed)
    ...
    async with (await get_context_pool()).acquire() as conn:
        await conn.execute(
            "UPDATE elephantq_jobs SET status='queued', attempts=0 WHERE id=$1",
            job_id,
        )
    ...
    async with (await get_context_pool()).acquire() as conn:
        row = await conn.fetchrow(
            "SELECT status, attempts, scheduled_at, last_error FROM elephantq_jobs WHERE id=$1",
            job_id,
        )
    ...
    async with (await get_context_pool()).acquire() as conn:
        row = await conn.fetchrow(
            "SELECT status, attempts FROM elephantq_jobs WHERE id=$1", job_id
        )
