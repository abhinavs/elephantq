...
Comprehensive tests for ElephantQ instance-based worker heartbeat system,
...
@pytest.mark.asyncio
async def test_worker_registration_instance_based(clean_db, elephantq_instance):
    """Test worker registration creates database record using ElephantQ instance"""
    ...
    async with pool.acquire() as conn:
        result = await conn.fetchrow(
            "SELECT * FROM elephantq_workers WHERE id = $1", heartbeat.worker_id
        )
...
@pytest.mark.asyncio
async def test_worker_heartbeat_updates_instance_based(clean_db, elephantq_instance):
    """Test heartbeat updates timestamp using ElephantQ instance"""
    app = elephantq_instance
    pool = await app.get_pool()
...
@pytest.mark.asyncio
async def test_worker_stop_heartbeat_instance_based(clean_db, elephantq_instance):
    """Test worker stopping marks status correctly using ElephantQ instance"""
    app = elephantq_instance
    pool = await app.get_pool()
    ...
    async with pool.acquire() as conn:
        result = await conn.fetchrow(
            "SELECT status FROM elephantq_workers WHERE id = $1", heartbeat.worker_id
        )
...
@pytest.mark.asyncio
async def test_worker_failure_and_recovery(clean_db, elephantq_instance):
    ...
    async with pool.acquire() as conn:
        stopped_workers = await conn.fetchval(
            "SELECT COUNT(*) FROM elephantq_workers WHERE status = 'stopped'"
        )
    ...
    async with pool.acquire() as conn:
        active_workers = await conn.fetchval(
            "SELECT COUNT(*) FROM elephantq_workers WHERE status = 'active'"
        )
...
async def test_database_pool_isolation_between_instances(clean_db, multiple_instances):
    """Test that database pools are properly isolated between ElephantQ instances"""
    ...
    for pool in [pool1, pool2, pool3]:
        async with pool.acquire() as conn:
            worker_count = await conn.fetchval("SELECT COUNT(*) FROM elephantq_workers")
    ...
    async def update_heartbeat_via_pool(pool):
        ...
        async with pool.acquire() as conn:
            await conn.execute(
                "UPDATE elephantq_workers SET last_heartbeat = NOW() WHERE id = $1",
                worker.worker_id,
            )
