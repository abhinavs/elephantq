...
async def test_enqueue_and_run_job():
    ...
    # Create a Soniq instance for testing (not using global)
...
async def test_cli_worker():
    ...
    # Test that we can access run_worker through global API (which uses Soniq instance)
    import soniq

    assert soniq.run_worker is not None

    # Test that Soniq instances have run_worker method
