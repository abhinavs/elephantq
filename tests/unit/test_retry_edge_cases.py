...
def test_backoff_with_zero_base_uses_one():
    ...
    result = compute_retry_delay_seconds(attempt=3, retry_delay=0, retry_backoff=True)
    assert result == 4.0  # 1.0 * 2^2


def test_backoff_with_none_delay_uses_one():
    result = compute_retry_delay_seconds(
        attempt=2, retry_delay=None, retry_backoff=True
    )
...
def test_max_delay_caps_backoff():
    result = compute_retry_delay_seconds(
        attempt=10, retry_delay=1, retry_backoff=True, retry_max_delay=100
    )
    assert result == 100.0
...
def test_list_delay_with_backoff():
    ...
    result = compute_retry_delay_seconds(
        attempt=2, retry_delay=[1, 3, 10], retry_backoff=True
    )
