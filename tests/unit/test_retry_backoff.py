...
def test_retry_backoff():
    assert (
        compute_retry_delay_seconds(attempt=1, retry_delay=1, retry_backoff=True) == 1
    )
    assert (
        compute_retry_delay_seconds(attempt=2, retry_delay=1, retry_backoff=True) == 2
    )
    assert (
        compute_retry_delay_seconds(attempt=3, retry_delay=1, retry_backoff=True) == 4
    )
