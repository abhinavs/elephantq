...
`ElephantQ(...)` instance method silently crossed database boundaries: the
...
async def test_contextvar_set_during_instance_method(monkeypatch):
    """ElephantQ instance methods attach self to the active-app contextvar.
    ...
    app = elephantq.ElephantQ(database_url="postgresql://localhost/test")
...
async def test_global_enqueue_uses_active_app_when_set():
    """elephantq.enqueue delegates to the active instance, not the global, when set."""
    from elephantq._active import _active_app

    class _FakeApp(elephantq.ElephantQ):
        # isinstance(active, ElephantQ) in _resolve_app picks us up only if we
    ...
    try:
        result = await elephantq.enqueue(task)
    finally:
...
async def test_global_enqueue_falls_back_to_global_when_no_active():
    ...
    with mock.patch("elephantq._get_global_app") as get_global:
