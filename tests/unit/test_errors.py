"""
Test suite for errors.py module - comprehensive coverage for ElephantQ exception handling.
"""

import pytest

from elephantq.errors import (
    ConfigurationError,
...
class TestElephantQError:
    """Test base ElephantQError class."""
    ...
    def test_error_with_documentation_url(self):
        ...
        doc_url = "https://docs.elephantq.dev/troubleshooting"
        error = ElephantQError(
    ...
    def test_error_complete_formatting(self):
        ...
        error_str = str(error)
        assert "ElephantQ Error [COMPLETE_ERROR]: Complete error" in error_str
        assert "Context:" in error_str
...
class TestDatabaseConnectionError:
    ...
    def test_database_connection_error_with_url(self):
        ...
        original_error = ConnectionError("Connection refused")
        database_url = "postgresql://user:secret123@localhost:5432/elephantq"
...
class TestCreateUserFriendlyError:
    ...
    def test_elephantq_error_passthrough(self):
        """Test that ElephantQError instances are returned as-is."""
    ...
    def test_generic_error_conversion(self):
        """Test conversion of generic errors to ElephantQError."""
        original_error = RuntimeError("Something went wrong")
        context = {"additional": "info"}

        result = create_user_friendly_error(original_error, context)

        assert isinstance(result, ElephantQError)
...
class TestErrorInheritance:
    """Test that all error classes properly inherit from ElephantQError."""

    def test_all_errors_inherit_from_elephantq_error(self):
        """Test that all custom error classes inherit from ElephantQError."""
        error_classes = [
            DatabaseConnectionError,
            JobNotFoundError,
            JobValidationError,
        ...
        for error_class in error_classes:
            assert issubclass(error_class, ElephantQError)
            assert issubclass(error_class, Exception)

    def test_error_can_be_caught_as_elephantq_error(self):
        """Test that specific errors can be caught as ElephantQError."""
        with pytest.raises(ElephantQError):
            raise JobNotFoundError("test_job")

    def test_error_can_be_caught_as_exception(self):
        """Test that ElephantQ errors can be caught as generic Exception."""
...
class TestErrorSerialization:
    ...
    def test_error_repr(self):
        ...
        error = ElephantQError("Test error", "TEST_CODE")

        # Should be able to create repr without errors
        repr_str = repr(error)
        assert "ElephantQError" in repr_str
